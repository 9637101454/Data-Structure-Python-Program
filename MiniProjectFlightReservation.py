...
class FlightReservationSystem:
    ...
    def list_flights(self):
        ...
        for f in self.flights:
            print(f"Flight {f.flight_no} | {f.origin} → {f.destination} | Seats: {f.seats}")
    ...
    def search_by_route(self, origin: str, destination: str):
        ...
        for f in self.flights:
            if f.origin.lower() == origin.lower() and f.destination.lower() == destination.lower():
                result.append(f"Flight {f.flight_no} | {f.origin} → {f.destination} | Seats: {f.seats}")
                found = True
        if not found:
...
def main():
    ...
    while True:
        ...
        output_history.clear()  # Clear previous outputs before each choice
        ...
        if choice == "1":
            ...
            if flight:
                new_output = [f"✅ Flight found: {flight}"]
            else:
            ...
            new_output = ["⚠ Invalid choice. Please try again."]
            output_history.extend(new_output)
            print("\n".join(output_history))
